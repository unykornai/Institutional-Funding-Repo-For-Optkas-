# CREDIT COMMITTEE POSITIONING BRIEF

**Borrower:** OPTKAS1 LLC
**Facility Request:** $2,400,000 – $4,000,000 Secured Credit Facility
**Date:** February 7, 2026
**Classification:** CONFIDENTIAL — For Credit Committee Use Only

---

## TRANSACTION OVERVIEW

OPTKAS1 LLC seeks a secured credit facility collateralized by investment-grade medium-term notes. The facility is structured as a standard borrowing-base loan with traditional UCC enforcement rights and independent custody verification.

---

## COLLATERAL SUMMARY

| Parameter | Value | Verification Source |
|:----------|:------|:--------------------|
| **Instrument** | TC Advantage 5% Secured Medium Term Notes | Subscription Agreement |
| **CUSIP** | 87225HAB4 | STC Position Statement |
| **ISIN** | US87225HAB42 | STC Verified |
| **Face Value** | $10,000,000 | STC Position Statement |
| **Coupon** | 5.00% per annum, annual | Certificate |
| **Maturity** | May 31, 2030 (4.3 years remaining) | Certificate |
| **Transfer Agent** | Securities Transfer Corporation (Plano, TX) | Industry standard |
| **Insurance** | (per PPM) blanket coverage (per PPM) | Premium invoice on file |
| **Form** | Physical certificate Book-entry (STC custody) | Chain of custody complete |

---

## BORROWING BASE

| Component | Amount | Advance Rate | Borrowing Base |
|:----------|:-------|:-------------|:---------------|
| TC Advantage 5% MTN | $10,000,000 | 40% | $4,000,000 |
| **Maximum Facility** | | | **$4,000,000** |

### Coverage Ratios

| Metric | Value | Threshold |
|:-------|:------|:----------|
| **Collateral Coverage** | 250% | Min 200% required |
| **Interest Coverage** | 125% | Min 100% required |
| **Maturity Buffer** | 4.3 years | Min 1 year required |

---

## WHY THIS DEAL IS APPROVABLE

### 1. The Collateral Is Real and Verifiable

- **STC-held** — Securities Transfer Corporation maintains position records
- **Physically delivered** — Certificate received, FedEx chain of custody documented
- **Insured** — (per PPM) per PPM coverage exceeds facility size by 6x
- **CUSIP registered** — 87225HAB4 is a verifiable identifier
- **Income-producing** — 5% coupon supports debt service at any advance rate up to 50%

### 2. The Enforcement Path Is Standard

- UCC-1 Financing Statement — ready to file
- Perfected security interest via STC Control Agreement
- Standard foreclosure and liquidation rights
- No cryptographic dependency for enforcement
- Traditional legal remedies fully available

### 3. The Borrower Infrastructure Exceeds Normal Standards

Unlike typical borrowers presenting static documents, OPTKAS1 LLC operates an automated infrastructure platform that provides:

| Capability | What It Means for the Lender |
|:-----------|:----------------------------|
| **Automated borrowing base certificates** | Monthly collateral reporting without manual preparation |
| **cryptographic cryptographic attestations** | Immutable, timestamped proof of collateral position |
| **Real-time compliance monitoring** | Covenant breaches detected automatically, not quarterly |
| **Live operational dashboard** | 16-card institutional monitor accessible on demand |
| **949 automated tests** | System integrity verified continuously, audit-grade evidence |
| **Exception reporting** | Unanchored events and compliance gate failures flagged immediately |

This infrastructure is **supplementary**. The deal stands on collateral alone. The platform simply makes your reporting, monitoring, and enforcement cleaner than anything you normally see.

---

## RISK ASSESSMENT

| Risk | Rating | Mitigation |
|:-----|:-------|:-----------|
| **Collateral authenticity** | Low | STC verification + physical certificate + CUSIP |
| **Collateral decline** | Low | 40% haircut absorbs substantial volatility |
| **Borrower default** | Low | First-lien security interest + control agreement |
| **Liquidity risk** | Medium | Rule 144A restriction; mitigated by 4.3-year maturity buffer |
| **Operational risk** | Very Low | Automated compliance, audit, and reporting infrastructure |
| **Documentation risk** | Very Low | Pre-executed templates, institutional data room, 34 documents |

---

## RECOMMENDED TERMS

| Term | Recommendation |
|:-----|:---------------|
| **Facility Size** | $2,400,000 – $4,000,000 |
| **Advance Rate** | 24% – 40% of face value |
| **Tenor** | 12 – 24 months |
| **Pricing** | SOFR + 800 – 1,200 bps |
| **Collateral** | First-priority perfected security interest in MTN |
| **Custody** | STC Control Agreement |
| **Covenants** | Minimum 200% coverage, quarterly reporting, no additional liens |
| **Reporting** | Monthly borrowing base certificate (automated), quarterly financial statements |

---

## DATA ROOM ACCESS

A complete institutional data room (34 documents, 7 categories) is available with role-based reading paths for:

- **Credit Fund Analysts** — 5-document fast track
- **Bank Credit Officers** — 6-document compliance path
- **External Legal Counsel** — 4-document enforcement review
- **Technical Due Diligence** — 4-document infrastructure audit

Contact: jimmy@optkas.com

---

## POSITIONING STATEMENT

> This is a straightforward borrowing-base facility secured by STC-verified, insured collateral with traditional UCC enforcement. The borrower's automated infrastructure platform makes verification and reporting cleaner than anything you normally see — but the deal stands on the collateral, not the technology.

---

*Prepared for credit committee review. All figures subject to due diligence verification. Data room available upon execution of NDA.*
