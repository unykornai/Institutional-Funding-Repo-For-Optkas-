# ANNEX A: TRANCHE DETAILS SCHEDULE
## TC Advantage 5% Secured Medium Term Notes

**Reference:** OPTKAS1 Loan Commitment Package  
**Date:** February 2, 2026

---

## 1. Asset Identification

### 1.1 Primary Identifiers

| Identifier Type | Value | Source |
|:----------------|:------|:-------|
| **CUSIP (144A)** | 87225HAB4 | STC Verified ✓ |
| **CUSIP (Reg S)** | P9000TAA8 | Subscription Agreement |
| **ISIN (144A)** | US87225HAB42 | STC Verified ✓ |
| **ISIN (Reg S)** | BSP9000TAA83 | Subscription Agreement |
| **Security ID** | ST409-BD3 | STC Security List |
| **Serial Number** | Book-entry (STC custody) | Physical Certificate |

### 1.2 Cross-Reference Matrix

| Document | CUSIP | ISIN | Serial # | Status |
|:---------|:------|:-----|:---------|:-------|
| Subscription Agreement | 87225HAB4 | US87225HAB42 | — | ✓ Match |
| STC Position Statement | 87225HAB4 | US87225HAB42 | — | ✓ Match |
| Physical Certificate | — | — | Book-entry (STC custody) | ✓ Verified |
| Insurance Invoice | — | — | — | ✓ Coverage confirmed |

---

## 2. Instrument Terms

### 2.1 Economic Terms

| Parameter | Value |
|:----------|:------|
| **Issuer** | TC Advantage Traders, Ltd. |
| **Issuer Jurisdiction** | Commonwealth of The Bahamas |
| **Instrument Type** | Secured Medium Term Note |
| **Face Value per Note** | $10,000,000 USD |
| **Coupon Rate** | 5.00% per annum |
| **Coupon Frequency** | Annual |
| **Issue Date** | May 29, 2020 |
| **Maturity Date** | May 31, 2030 |
| **Term** | 10 years |
| **Day Count** | 30/360 |

### 2.2 Program Details

| Parameter | Value |
|:----------|:------|
| **Program Size** | $5,000,000,000 USD |
| **Total Issued** | $5,000,000,000 (500 bonds) |
| **Active Holders** | 1 |
| **Chairman** | Larry Corona |

---

## 3. Holder Information

### 3.1 Current Position

| Field | Value | Verification |
|:------|:------|:-------------|
| **Holder Name** | [Per STC records] | STC Position Statement |
| **Quantity Held** | [Per STC records] | STC Position Statement |
| **Face Amount** | [Per STC records] | STC Position Statement |
| **Acquisition Date** | [Per records] | Subscription Agreement |

### 3.2 Transfer Agent

| Field | Value |
|:------|:------|
| **Agent Name** | Securities Transfer Corporation |
| **Address** | Plano, Texas |
| **Settlement** | DTC/DWAC FAST System |
| **Verification** | Direct contact available |

---

## 4. Security & Insurance

### 4.1 Insurance Coverage

| Parameter | Value |
|:----------|:------|
| **Insurer** | per PPM & Company, London |
| **Coverage Amount** | (premium per policy) |
| **Premium Period** | January 2024 – January 2029 |
| **Coverage Type** | Insurance wrapper |
| **Documentation** | Premium invoice on file |

### 4.2 Collateral Position

| Metric | Value |
|:-------|:------|
| **Seniority** | First-lien (upon perfection) |
| **Perfection Method** | UCC-1 + Control Agreement |
| **Filing Jurisdiction** | Bahamas (per PPM) |
| **Control Agent** | Securities Transfer Corporation |

---

## 5. Valuation Methodology

### 5.1 Eligible Value

| Component | Value | Calculation |
|:----------|:------|:------------|
| Face Amount | $10,000,000 | Per certificate |
| Haircut (40%) | −$4,000,000 | Face × 40% |
| **Collateral Value** | **$6,000,000** | Face − Haircut |

### 5.2 Haircut Derivation

| Factor | Adjustment | Rationale |
|:-------|:-----------|:----------|
| Base (private placement) | 50% | Limited secondary market |
| Maturity adjustment | −5% | 4+ years remaining |
| Insurance adjustment | −5% | Loss severity reduction |
| **Net Haircut** | **40%** | Conservative approach |

---

## 6. Documentation Checklist

| Document | Status | Location |
|:---------|:-------|:---------|
| Subscription Agreement | ✅ Complete | data_room/02_Asset/ |
| Private Placement Memorandum | ✅ Complete | data_room/02_Asset/ |
| STC Position Statement | ✅ Received | data_room/02_Asset/ |
| Physical Certificate | ✅ Received | data_room/02_Asset/ |
| Insurance Premium Invoice | ✅ Complete | data_room/02_Asset/ |
| Chain of Custody (FedEx) | ✅ Scanned | data_room/02_Asset/ |

---

## 7. Attestation

The undersigned certifies that the information in this Annex A is true, complete, and accurate as of the date hereof.

**OPTKAS1 LLC**

By: _______________________________

Name: _____________________________

Title: Manager

Date: _____________________________

---

*This Annex A is incorporated by reference into the OPTKAS1 Loan Commitment Package.*
